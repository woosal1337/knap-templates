...
[![CI](https://github.com/woosal1337/knap-templates/actions/workflows/ci.yml/badge.svg)](https://github.com/woosal1337/knap-templates/actions/workflows/ci.yml)
...
Version 1.0.0 is the first public release. Automated and desktop acceptance checks pass. Mobile acceptance is not yet verified.
...
4. Select the pencil action to edit the source, or select the file-output action to create a note.
5. Select the reading-view button to return to the preview, or review the data and output path and select **Create note**.

The plugin also adds **Render with Knap** to the file menu for templates in a configured folder.

## Template detection

Configured template folders are the explicit discovery boundary. Within those folders, the plugin treats `.md` and `.knap` files as templates, except files named `README`.

The plugin does not scan the full vault. Knap defines template syntax and rendering, but it does not define an Obsidian file-discovery marker. Keep ordinary notes outside configured template folders.
...
Version 1.0.0 is the first GitHub release. Complete the mobile checks in `docs/test-plan.md` before a Community directory request.
